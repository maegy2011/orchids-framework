...
export default function WatchClient({ 
  ...
  const [sidebarOpen, setSidebarOpen] = useState(false);
    ...
    const [autoPauseOnNote, setAutoPauseOnNote] = useState(true);
  ...
  const quickCapture = () => {
    ...
    const startCapture = () => {
      if (autoPauseOnNote && isPlayerInteractive()) {
        playerRef.current.pauseVideo();
      }
      const time = getCurrentPlayerTime();
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-6" dir={direction}>
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-6 p-4" dir={direction}>
        <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center">
          <AlertCircle size={32} className="text-red-600" />
        </div>
        <div className="text-center max-w-md">
          <h1 className="text-xl font-bold text-[#0f0f0f] mb-2">{t('retry')}</h1>
  ...
  if (blocked) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-6 p-4" dir={direction}>
        ...
        <div className="text-center max-w-md">
          <h1 className="text-2xl font-bold text-[#0f0f0f] mb-2">{t('content_blocked')}</h1>
  ...
  if (!video) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center gap-6 p-4" dir={direction}>
        ...
        <div className="text-center max-w-md">
          <h1 className="text-xl font-bold text-[#0f0f0f] mb-2">{t('video_not_found')}</h1>
          <p className="text-[#606060]">{t('video_not_found_desc')}</p>
        </div>
        <Link 
          href="/" 
          className="flex items-center gap-2 px-6 py-3 bg-red-600 text-white rounded-full hover:bg-red-700 transition-colors"
        ...
        <div className="max-w-[1800px] mx-auto flex flex-col lg:flex-row gap-0 lg:gap-6">
              ...
              <div className="relative w-full aspect-video bg-gradient-to-br from-indigo-900 via-purple-900 to-red-900 lg:rounded-2xl overflow-hidden mb-4 flex items-center justify-center shadow-2xl">
                ...
                <div className="text-center text-white z-10 px-6">
                  ...
                  <h2 className="text-2xl font-bold mb-2">{t('listening_mode_active')}</h2>
                  <p className="text-gray-300 max-w-xs mx-auto text-sm">{t('listening_mode_desc')}</p>
                  <button
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                      ...
                      <p className="text-xs text-[#606060] font-medium">{video.channelSubscribers} {t('subscribers')}</p>
                    </div>
                    <button 
                ...
                <div className="flex items-center gap-3 overflow-x-auto no-scrollbar pb-2 sm:pb-0">
                  ...
                  >
                    <Clock size={18} className={isInWatchLater(videoId) ? "text-white" : "text-[#0f0f0f]"} />
                  ...
                  >
                    ...
                    <span className="text-xs font-bold hidden sm:inline">
                      {useAlternativePlayer ? t('default_player') : t('alternative_player')}
                    </span>
                  </button>
                  ...
                  {!backgroundPlayEnabled && (
                    ...
                    >
                      ...
                      <span className="text-xs font-bold">{t('audio_mode')}</span>
                    </button>
                  )}
                  ...
                  <div className="bg-[#f8f8f8] border border-gray-200 rounded-2xl p-4 mb-8 shadow-sm group">
                    <div className="flex items-center gap-3 text-sm text-[#0f0f0f] font-bold mb-3">
                      ...
                      <span className="bg-white px-3 py-1 rounded-full border border-gray-200 shadow-sm">{formatDate(video.uploadDate, language)}</span>
                    </div>
